...
####### split input
splitDir <- file.path(PATH, Aligner, paste0("split_", sampleName))
...
Sys.time()
# Step 1: Summarize isoform-level info
iso_info <- df1%>%
  group_by(SampleID, isoform) %>%
  summarise(
    cds_len    = sum(CDS_end - CDS_start + 1),
    nblock     = n(),
    exon_order = paste(sort(as.numeric(order)), collapse = "-"),
    continuous = all(diff(sort(as.numeric(order))) == 1),
    pos_sig    = paste(CDS_chr, CDS_start, CDS_end, CDS_strand, sep = ":", collapse = ";"),
    .groups = "drop"
  )

# Step 2: For each SampleID, select isoforms according to your rules
chosen_isoforms <- iso_info %>%
  group_by(SampleID) %>%
  mutate(
    has_cont = any(continuous)
  ) %>%
  group_modify(~{
    x <- .x
    if (x$has_cont[1]) {
      # Keep only continuous isoforms
      iso_sub <- x %>% filter(continuous)
...
# Step 3: Join back to df1 to get row-level info
df1_summary <- df1 %>%
  inner_join(chosen_isoforms %>% unnest(cols = c(chosen_isoforms)), 
             by = c("SampleID", "isoform" = "chosen_isoforms")) %>%
  group_by(SampleID) %>%
  summarise(
    gene       = first(gene),
    gene_strand= first(strand),
    isoform    = paste(unique(isoform), collapse = "||"),
    nblock     = n_distinct(order),
    NO.Exon    = paste(sort(unique(as.numeric(order))), collapse = "-"),
    position   = paste(paste(CDS_chr, CDS_start, CDS_end, CDS_strand, sep=":"), collapse=";"),
    fusion     = "N",
    note       = if(any(continuous)) "continuous CDS and edge-matching" else "discontinuous CDS",
    type       = if(any(continuous)) "normal" else "novel with deletion",
    .groups    = "drop"
  )
Sys.time()
